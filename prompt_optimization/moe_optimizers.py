...
class MOEProTeGi(PromptOptimizer):
    """ MOEProTeGi: Mixture of experts Prompt Optimization with Textual Gradients"""
    def __init__(self, args, evaluator_fn, scorer, max_threads=1, bf_eval=None):
    ...
    def _sample_error_str(self, texts, labels, preds, task, n=4):
        ...
        for i, (t, l, p) in enumerate(zip(sample_texts, sample_labels, sample_preds)):
            ...
            error_string += f'Input Text: \"{t.strip()}\"\Correct answer: {task.stringify_prediction(l)}\nModel prediction: {task.stringify_prediction(p)}\n\n'
            error_idx += 1
    ...
    def expand_candidates_phase_1(self, prompts, task, gpt4, texts, labels, preds):
